				      ...
				      unsigned long flags)
{
	...
	if (buffer->flags & ION_FLAG_PROTECTED) {
		...
		info->prot_desc.chunk_size = (unsigned int)len;
		info->prot_desc.bus_address = paddr;
		ret = ion_secure_protect(buffer);
		if (ret)
			goto err_free_mem;
	} else if (carveout_heap->untouchable) {
		/*
		 * Since that heap is inaccessible for speical use,
		 * it should always be declared as protected
		 * to prevent any accessing the buffer from driver.
		 */
		pr_err("%s: Invalid flag %lx on untouchable heap(%d) for %s\n",
		       __func__, buffer->flags, buffer->heap->id,
			   buffer->heap->name);
		ret = -EINVAL;
	...
	return 0;
err_free_mem:
	ion_carveout_free(buffer->heap, paddr, len);
err_free_table:
...
static void ion_carveout_heap_free(struct ion_buffer *buffer)
{
	...
	ion_carveout_free(buffer->heap, sg_phys(buffer->sg_table->sgl), len);
	sg_free_table(&info->table);
	kfree(info);
}
